# Advanced Regression
'''Ref: https://www.kaggle.com/lavanyashukla01/getting-started-with-kaggle-explore-the-dataset?scriptVersionId=13419764'''
...
# Read CSVs
fetch_train = 'D:\Programming Tutorials\Machine Learning\Projects\Datasets\House Price Adv Regression train.csv'
train = pd.read_csv(fetch_train)

fetch_test = 'D:\Programming Tutorials\Machine Learning\Projects\Datasets\House Price Adv Regression test.csv'
test = pd.read_csv(fetch_test)
...
# checking if the training and test sets seem to have consistently distributed features
...
def percentage_blocks(df, var):
    """
    Take as input a dataframe and variable, and return a Pandas series with approximate percentage values for 
    filling out a waffle plot.
    """
    # compute base quotas
...
'''Third, we need to do involve aesthetic cleaning to polish it up for interpretability. We can take away the 
plot border and ticks, since they're arbitrary, but we should change the limits so that the boxes are square.
We should also add a legend so that the mapping from colors to category levels is clear.
# aesthetic wrangling'''
